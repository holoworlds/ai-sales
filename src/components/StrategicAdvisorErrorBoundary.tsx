...
export default class StrategicAdvisorErrorBoundary extends Component<Props, State> {
  public state: State = {
    hasError: false,
    error: null,
    countdown: 0
  ...
  public componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error('[StrategicAdvisorErrorBoundary] Uncaught error:', error, errorInfo);
      ...
      return (
        <div className="h-full flex items-center justify-center p-12 bg-[#0F1113] rounded-[3.5rem] border border-white/5 shadow-2xl">
          <div className="max-w-md w-full bg-[#1A1C1E] border border-red-500/20 p-12 rounded-[2.5rem] text-center space-y-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-white mb-2">战略咨询模块出现异常</h2>
              <p className="text-xs text-gray-500 leading-relaxed">
                组件在执行决策推理时遇到了同步冲突或状态循环。这通常是因为短时间内触发了过多的渲染尝试。
              </p>
            </div>
            {this.state.error && (
