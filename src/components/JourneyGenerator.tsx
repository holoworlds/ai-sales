          ...
          <AnimatePresence mode="wait">
            {!insight ? (
              <motion.div 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="flex-1 border-2 border-dashed border-gray-200 rounded-[3rem] flex flex-col items-center justify-center p-20 text-center"
              >
                ...
                >
                   ...
                   <div className="relative z-10">
                      <div className="flex items-center justify-between mb-10">
                         ...
                         <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Judgment: {insight.currentAnalysis.stage}</div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                         <div className="space-y-8">
                            <div>
                               ...
                               <div className="text-xl font-bold leading-relaxed">{insight.currentAnalysis.intent}</div>
                            </div>
                            <div>
                               <label className="text-[9px] font-black uppercase tracking-[0.2em] text-gray-500 mb-3 block">当前目标</label>
                               <div className="text-lg text-blue-200 font-medium">{insight.currentAnalysis.objective}</div>
                            </div>
                            <div className="p-6 bg-white/5 border border-white/10 rounded-2xl">
                               <label className="text-[9px] font-black uppercase tracking-[0.2em] text-gray-500 mb-3 block">回答策略</label>
                               <div className="text-sm leading-relaxed text-gray-300">{insight.currentAnalysis.strategy}</div>
                            </div>
                         </div>
                         ...
                         <div className="bg-white text-[#1A1C1E] rounded-[2rem] p-8 shadow-inner flex flex-col justify-between">
                            <div>
                               <div className="flex items-center justify-between mb-6">
                                  ...
                                  <button onClick={() => navigator.clipboard.writeText(insight.currentAnalysis.suggestedScript)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                                     <Copy className="w-4 h-4 text-gray-400" />
                                  </button>
                               </div>
                               <div className="text-xl font-bold leading-[1.6]">
                                  <ReactMarkdown>{insight.currentAnalysis.suggestedScript}</ReactMarkdown>
                               </div>
                            </div>
                                ...
                                <div className="flex items-center gap-6">
                                   ...
                                   <div>
                                      <div className="flex items-center gap-3 mb-1">
                                         <h4 className={`text-base font-bold ${isCurrent ? 'text-blue-900' : 'text-gray-900'}`}>{step.stage}</h4>
                                         {isCurrent && (
                                           <span className="px-2 py-0.5 bg-blue-500 text-white text-[8px] font-black uppercase tracking-widest rounded">Current Focus</span>
                                         )}
                                      </div>
                                      <p className="text-xs text-gray-500 font-medium">{step.definingTraits}</p>
                                   </div>
                                </div>
                                       ...
                                       <div className="space-y-6">
                                          ...
                                          <div className="p-6 bg-[#1A1C1E] text-white rounded-3xl relative overflow-hidden">
                                             ...
                                             <div className="text-sm font-bold leading-relaxed">
                                                <ReactMarkdown>{step.suggestedScript}</ReactMarkdown>
                                             </div>
                                          </div>
                                       </div>
                                    </div>
